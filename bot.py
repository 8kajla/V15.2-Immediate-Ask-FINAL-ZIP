...
def restore_strategy_distribution_from_ledger():
    """Restore strategy-band allocation from durable signal metadata.

    The execution price may be in a different market price segment than the
    signal price. Therefore restoration must use the stored V15.2 fine_band /
    target_capital, never reclassify a signal from its later execution price.
    """
    observed = set()
    for trade in getattr(ledger, "trades", []):
        if trade.get("action") != "BUY":
            continue
        meta = trade
        key = meta.get("signal_key") or meta.get("order_id") or f"{trade.get('condition')}|{trade.get('token')}|{trade.get('ts')}|{trade.get('price')}|{trade.get('notional')}"
        key = str(key)
        if key in observed:
            continue
        band = meta.get("fine_band")
        notional = meta.get("target_capital")
        if band is None:
            try:
                band, _ = strategy.fine_band(float(trade.get("price")))
            except (TypeError, ValueError):
                band = None
        if notional is None:
            try:
                raw = float(trade.get("notional", 0.0))
                notional = raw / LIVE_SIZE_SCALE if EXECUTION_MODE and LIVE_SIZE_SCALE > 0 else raw
            except (TypeError, ValueError):
                notional = 0.0
        if band and float(notional or 0.0) > 0:
            try:
                strategy.observe_trade_distribution(str(band), float(notional))
                observed.add(key)
            except ValueError:
                continue
        ...
        try:
            ...
            for market in market_list:
                ...
                meta = {
                    ...
                    "regime": regime,
                    "fine_band": band,
                    "execution_mode": "V15.2_IMMEDIATE_ASK",
                    ...
                    try:
                        ...
                        if plan is None:
                            p(f"CLOB IMMEDIATE ASK WAIT | asset={market['asset']} | side={signal.side} "
                              ...
                              f"| reason=NO_EXECUTABLE_ASK_BATCH")
                            continue
                        ...
                        if required_cash > available_cash + 1e-9:
                            p(f"CLOB IMMEDIATE ASK CAPITAL WAIT | asset={market['asset']} | required=${required_cash:.4f} | available=${available_cash:.4f}")
                            continue
                        ...
                        selected_meta.update({
                            "execution_mode": "CLOB_IMMEDIATE_ASK_FAK",
                        ...
                        p(f"LIVE ORDER SUBMITTED | mode=CLOB_IMMEDIATE_ASK_FAK | order_id={order_id} | asset={market['asset']} "
                        ...
                        if any(x in msg for x in ("below market minimum", "market minimum", "no acceptable ask")):
                            p(f"CLOB IMMEDIATE ASK WAIT | asset={market['asset']} | reason={exc}")
                            continue
                ...
                # Distribution is a STRATEGY invariant. Record the original V15.2
                # signal band/notional, never the eventual execution-price band.
                # This keeps the trader model unchanged even when the live ask
                # is above/below the signal price.
                if EXECUTION_MODE:
                    for selected in getattr(executed_order.get("plan") if executed_order else None, "items", ()):
                        smeta = selected.get("meta") or {}
                        sband = smeta.get("fine_band") or band
                        sraw = smeta.get("raw_signal_notional")
                        if sraw is None:
                            sval = float(selected.get("notional", 0.0))
                            sraw = sval / LIVE_SIZE_SCALE if LIVE_SIZE_SCALE > 0 else sval
                        if sband and float(sraw) > 0:
                            strategy.observe_trade_distribution(str(sband), float(sraw))
                    if not (executed_order and getattr(executed_order.get("plan"), "items", ())):
                        # Defensive fallback; this should only occur outside the
                        # adaptive execution path. Keep distribution in the V15.2
                        # virtual $300 capital frame.
                        observed_notional = raw_notion
                        strategy.observe_trade_distribution(band, observed_notional)
