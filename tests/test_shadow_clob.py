...
def test_shadow_immediate_ask_ignores_signal_limit_and_consumes_visible_book(tmp_path):
    s = ShadowCLOB(tmp_path / "shadow.json")
    fake = FakeSession()
    fake.book["asks"] = [
        {"price": "0.55", "size": "2"},
    ...
    s.session = fake
    # 5 requested shares @ best ask = $2.75 strategy execution budget.
    # The signal limit is deliberately far below the available ask.
    response = s.adaptive_buy("t1", 0.39, 5.0, "c1")
    assert response["success"] is True
    assert response["status"] == "matched"
    assert response["takingAmount"] == pytest.approx(5.0)
    assert response["vwap"] == pytest.approx((2.0 * .55 + 3.0 * .56) / 5.0)
    assert response["unfilled_shares"] == pytest.approx(0.0)


def test_shadow_immediate_fak_partial_releases_remainder(tmp_path):
    s = ShadowCLOB(tmp_path / "shadow.json")
    fake = FakeSession()
    fake.book["asks"] = [{"price": "0.55", "size": "2"}]
    s.session = fake
    response = s.adaptive_buy("t1", 0.20, 5.0, "c1")
    oid = response["orderID"]
    assert response["takingAmount"] == pytest.approx(2.0)
    assert response["unfilled_shares"] == pytest.approx(3.0)
    order = s.orders[oid]
    assert order["status"] == "PARTIAL_CANCELED"
    assert order["remaining_shares"] == pytest.approx(0.0)
    fill = s.get_trades()[0]
    assert fill["order_final_status"] == "PARTIAL_CANCELED"
    assert fill["unfilled_shares"] == pytest.approx(3.0)
    assert s.get_trades() == []
