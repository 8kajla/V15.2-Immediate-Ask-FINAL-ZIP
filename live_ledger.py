...
class LiveLedger:
    ...
    def sync_trades(self, trades):
        ...
        for trade in trades or []:
            ...
            final_status = str(trade.get("order_final_status") or "").upper()
            if final_status in {"CANCELED", "CANCELLED", "PARTIAL_CANCELED", "FAILED"}:
                # FAK/IOC unmatched remainder is gone immediately. Do not carry
                # its original reservation forward to later loops/restarts.
                order["status"] = "CLOSED_OR_CANCELED"
                order["cancel_reason"] = final_status
                order["released_reserved"] = max(0.0, remaining_reserved)
            else:
